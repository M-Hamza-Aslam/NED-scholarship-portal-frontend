import classes from "./MeritScholarshipForm.module.css";
...
const MeritScholarshipForm = (props) => {
  ...
  const {
    value: matricPercentageInputValue,
    isValid: enteredMatricPercentageisValid,
    isError: matricPercentageIsError,
  ...
  const {
    value: intermediatePercentageInputValue,
    isValid: enteredIntermediatePercentageisValid,
    isError: intermediatePercentageIsError,
  ...
  const {
    value: bachelorCGPAInputValue,
    isValid: enteredBachelorCGPAisValid,
    isError: bachelorCGPAIsError,
  ...
  const formSubmitHandler = async (event) => {
    try {
      ...
      const res = await fetch(
        isCreating
          ? `${BACKEND_DOMAIN}/alumni/create-merit-scholarship`
          : `${BACKEND_DOMAIN}/alumni/update-merit-scholarship?scholarshipId=${scholarshipDetails._id}`,
        {
          method: "POST",
          headers: {
      ...
      if (scholarshipImg.value) {
        ...
        const resImg = await fetch(
          `${BACKEND_DOMAIN}/alumni/upload-scholarshipImg`,
          {
            method: "POST",
            headers: {
              Authorization: "Bearer " + token,
      ...
      //resetting inputs
      resetTitleInput();
      resetMatricPercentageInput();
      resetIntermediatePercentageInput();
      resetBachelorCGPAInput();
  ...
  return (
    <form onSubmit={formSubmitHandler}>
      ...
      <TextField
        id="outlined-adornment-matricPercentage"
        label="Matric percentage"
        error={matricPercentageIsError && true}
        // size="small"
        className={classes.formInput}
        type="number"
        value={matricPercentageInputValue}
        onChange={matricPercentageKeyStrockHandler}
        onBlur={matricPercentageInputBlurHandler}
        helperText={
          matricPercentageIsError
            ? "Incorrect Percentage!"
            : "Enter required percentage in Matric"
        }
      />
      <TextField
        id="outlined-adornment-intermediatePercentage"
        label="Intermediate percentage"
        error={intermediatePercentageIsError && true}
        // size="small"
        className={classes.formInput}
        type="number"
        value={intermediatePercentageInputValue}
        onChange={intermediatePercentageKeyStrockHandler}
        onBlur={intermediatePercentageInputBlurHandler}
        helperText={
          intermediatePercentageIsError
            ? "Incorrect Percentage!"
            : "Enter required percentage in Intermediate"
        }
      />
      <TextField
        id="outlined-adornment-bachelorCGPA"
        label="Bachelor CGPA"
        error={bachelorCGPAIsError && true}
        // size="small"
        className={classes.formInput}
        type="number"
        value={bachelorCGPAInputValue}
        onChange={bachelorCGPAKeyStrockHandler}
        onBlur={bachelorCGPAInputBlurHandler}
        helperText={
          bachelorCGPAIsError
            ? "Incorrect CGPA!"
            : "Enter required CGPA in Bachelor"
        }
      />
      <TextField
      ...
      <div className={classes.btnDiv}>
        <Button
          ...
          disabled={!isSubmit}
        >
          {isCreating ? "Request Scholarship" : "Update Scholarship"}
        </Button>
        <Button
          type="button"
          variant="outlined"
...
export default MeritScholarshipForm;
